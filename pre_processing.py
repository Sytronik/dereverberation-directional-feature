    ...
    for fname in all_files:
        ...
        for i_proc in range(N_CORES):
            if (i_proc + 1)*n_loc_per_core <= N_LOC:
                range_loc = range(i_proc*n_loc_per_core, (i_proc + 1)*n_loc_per_core)
...
def save_IV(i_dev: int,
    ...
    for i_loc in range_loc:
        ...
        for i_frame in range(N_frame_free):
            ...
            iv_free[:, i_frame, :3] = calc_intensity(anm[:, :N_freq], *sftdata_cp.get_triags())
            iv_free[:, i_frame, 3] = cp.sum(cp.abs(anm[:, :N_freq])**2, axis=0)
        ...
        for i_frame in range(N_frame_room):
            ...
            iv_room[:, i_frame, :3] = calc_intensity(anm[:, :N_freq], *sftdata_cp.get_triags())
            iv_room[:, i_frame, 3] = cp.sum(cp.abs(anm[:, :N_freq])**2, axis=0)
