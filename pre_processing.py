...
def process(DIR_WAVFILE:str, ID:str, N_START,
    ...
    for folder, _, _ in os.walk(DIR_WAVFILE):
        ...
        for file in files:
            ...
            Nframe_free = int(np.floor(L_data_free/L_HOP)-1)
...
def save_IV(data, RIR,
    ...
    IV_free = cp.zeros((int(N_FFT/2), Nframe_free, 4))
    for i_frame in range(Nframe_free):
        ...
        IV_free[:, i_frame, :3] \
                = calcIntensity(anm_free[:, :int(N_FFT/2)], Wnv, Wpv, Vv)
    ...
    IV_room = cp.zeros((int(N_FFT/2), Nframe_room, 4))
    for i_frame in range(Nframe_room):
        ...
        IV_room[:, i_frame, :3] \
                = calcIntensity(anm_room[:, :int(N_FFT/2)], Wnv, Wpv, Vv)
        IV_room[:, i_frame, 3] \
                = np.abs(anm_room[0, :int(N_FFT/2)])
